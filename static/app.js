...
async function updateCode() {
    if (!confirm("¿Estás seguro de que querés actualizar el código del bridge desde GitHub y reiniciar el servicio?")) {
        return;
    }

    const btn = document.getElementById("btn-update-code");
    btn.classList.add("btn--loading");
    btn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Actualizando...`;

    logEvent("info", "Iniciando actualización de código desde GitHub...");

    const data = await apiCall("POST", "/api/system/update");

    btn.classList.remove("btn--loading");
    btn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Actualizar Código`;

    if (data && data.success) {
        logEvent("success", data.message);
        if (data.output) {
            logEvent("info", `Git Output:\n${data.output}`);
        }
        logEvent("warn", "Reiniciando el servicio... La conexión con el panel se perderá temporalmente.");

        // Mostrar estado de reconexión
        setConnectionStatus("checking", "Reiniciando...");
        
        // Polling para esperar que el servicio vuelva
        setTimeout(() => {
            const checkInterval = setInterval(async () => {
                const res = await apiCall("GET", "/api/health");
                if (res) {
                    clearInterval(checkInterval);
                    logEvent("success", "Servicio reconectado exitosamente.");
                    await refreshStatus();
                }
            }, 3000);
        }, 2000);
    } else {
        const errorDetail = data?.error || data?.message || "Error desconocido";
        logEvent("error", `Error al actualizar: ${errorDetail}`);
    }
}
